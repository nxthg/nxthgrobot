...
import de.nxthg.fahrer.GreiferEvents;
//import de.nxthg.greifer.AufAbladenV2.ConnectorLift;
 ...
 *
 */
public class NXTHGNXTNavigationModel extends NXTHGNavigationModel implements MoveListener, NavigationListener, WaypointListener, FeatureListener {
	...
	class Receiver implements Runnable {
				...
				try {
					...
					synchronized(this) {					
						...
						case AUFLADEN_PAKET: // Request to travel an arc og given radius and angle
							...
							if (groesse==1){
								if (regal==1){
									dosGreifer.write(GreiferEvents.KISTE_KLEIN_UNTEN.ordinal());
								}
							}
							
							if (groesse==2){
								if (regal==1){
									dosGreifer.write(GreiferEvents.KISTE_MITTEL_UNTEN.ordinal());
								}
							}
							
							if (groesse==2){
								if (regal==2){
									dosGreifer.write(GreiferEvents.KISTE_MITTEL_MITTE.ordinal());
								}
							}
							
							if (groesse==3){
								if (regal==3){
									dosGreifer.write(GreiferEvents.KISTE_GROSS_OBEN.ordinal());
								}
							}
		...
		class ConnectorGreifer implements Runnable {
			public void run() {
				...
				System.out.flush();
				disGreifer = conn.openDataInputStream();
				dosGreifer = conn.openDataOutputStream();
				running=true;
				while(running) {
					try {
						...
						synchronized(this) {
							...
							case FAHR_VOR:
								...
								dosGreifer.write(GreiferEvents.FAHRER_VORNE.ordinal());
								break;
								
							case FAHR_ZURUECK:
								navigator.getMoveController().travel(-5);
								dosGreifer.write(GreiferEvents.FAHRER_HINTEN.ordinal());
								break;
							}								
						}			
					}
